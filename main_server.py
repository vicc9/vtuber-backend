...
async def websocket_endpoint(
    ...
    try:
        while True:
            ...
            # 情況 1：直接收到二進位音訊 (保留原本功能，以防萬一)
            if message.get("bytes"):
                audio_bytes = message["bytes"]
                print(f"🎤 收到二進位音訊，大小: {len(audio_bytes)} bytes")
                await handle_audio_input(audio_bytes, websocket, send_lock, reset_idle_timer)

            # 情況 2：收到文字訊息 (可能是 JSON 包含 Base64 音訊，也可能是純文字對話)
                ...
                try:
                    # 嘗試解析 JSON (Unity 傳來的格式通常會包裝成 JSON)
                    data = json.loads(raw_text)
                    
                    # 檢查是否為 Base64 音訊傳輸
                    if "audio_base64" in data:
                        # 將 Base64 字串解碼轉回 Bytes
                        audio_bytes = base64.b64decode(data["audio_base64"])
                        print(f"🎤 收到 Base64 音訊並解碼，大小: {len(audio_bytes)} bytes")
                        await handle_audio_input(audio_bytes, websocket, send_lock, reset_idle_timer)
                    ...
                    # 如果不是 JSON，就當作純文字對話處理
                    print(f"\n💬 觀眾說 (純文字): {raw_text}")
                    reset_idle_timer("text")
                    await process_and_respond(websocket, raw_text, send_lock)
                    reset_idle_timer("text")
...
# 共用：處理音訊輸入
# ─────────────────────────────────────────
async def handle_audio_input(audio_bytes: bytes, websocket: WebSocket, send_lock: asyncio.Lock, reset_idle_timer):
    """處理接收到的音訊：辨識 STT 並回覆"""
    reset_idle_timer("voice")
    
    # 提醒前端正在辨識中
    async with send_lock:
        await websocket.send_text(json.dumps({"type": "stt_status", "status": "recognizing"}))
    ...
    # 回傳辨識結果給前端
    async with send_lock:
        await websocket.send_text(json.dumps({"type": "stt_result", "text": user_text}))
